...
func GetData() ([]Racktables, error) {
	...
	statement := "SELECT id, name, label, objtype_id, asset_no, has_problems, comment FROM rackobject"

	// to be filled in with appropriate user, password, and db name.
	db, err := sql.Open("mysql", "user:password@/racktables")
	...
	for results.Next() {
		...
		resType := determineIDMeaning(rt.ID, rt.Name)
		rt.Type = resType
		typeID := &rt.ObjtypeID
		rt.IP = GetIPDetaiLs(*typeID)

		portInfo := GetPortDetails(*typeID)
		rt.Port = portInfo
		rackIDinfo := GetRackDetails(*typeID)
		rt.RackID = rackIDinfo
...
func GetRackDetails(objectID string) string {
	var rt Racktables
